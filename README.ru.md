...
Gradle подтягивает всё сам — toolchain JDK 21, платформу IntelliJ IDEA 2025.3
и Node для сборки анализатора — так что настраивать `JAVA_HOME` или SDK не нужно
(для запуска Gradle подойдёт любой JDK). Плагин пока не опубликован в
маркетплейсе — ставится из собранного дистрибутива.

## Демо

`examples/reatom-demo` — небольшой проект-потребитель `@reatom/core`, игрушечная
корзина покупок: `atom` / `computed` / `action` / `effect`, расширение и связи
между файлами. Откройте его в IDE с плагином, чтобы увидеть Code Lens и
gutter-иконки:
...
`packages/ide-plugin/scripts/start-sandbox.sh` поднимает песочницу IDE с уже
установленным плагином и по умолчанию открывает это демо.
